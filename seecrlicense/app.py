...
Example:
{
    ...
    supportedTypes="; ".join(
        ["*" + ext for ext in list(markersByExtension.keys())]
        + list(markersByFilename.keys())
    ),
...
def main(args=None):
    ...
    parser.add_argument(
        "configPath", metavar="<configFile>", help="path to config file."
    )
    parser.add_argument(
        "path",
        metavar="<file|directory>",
        help="file or directory to which license needs to be applied.",
        nargs="+",
    )
    parser.add_argument(
        "--force",
        ...
        default=False,
        dest="forceUpdate",
        help="""Use this to force the license onto the files. This can be handy when files have been moved from other projects but their copyright holders have already been updated. By forcing the license onto the file they project name and description in the license will be updated.""",
    )
    parser.add_argument(
        "--changed-only",
        action="store_true",
    ...
    parser.add_argument(
        ...
        default=None,
        dest="year",
        help="Use given year in license or current year if not set",
    )
    parser.add_argument(
        "--select",
        help='Select which copyright holders should be applied. Comma separated. ie: "seecr,cq2"',
    )
